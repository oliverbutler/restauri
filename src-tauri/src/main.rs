...
use sqlx::{sqlite::{SqlitePoolOptions, SqliteConnectOptions}, Pool, Row, Sqlite, SqlitePool, pool::{self, PoolConnection}};
use ts_rs::TS;
...
async fn add_request_history(pool: &tauri::State<'_, Pool<Sqlite>>, request_history: RequestHistory) -> sqlx::Result<()> {
...
async fn add_request(pool: tauri::State<'_, Pool<Sqlite>>, name: &str) -> Result<String, String> {
    ...
    sqlx::query_as!(Request, "INSERT INTO requests (name, url, method, headers) VALUES (?, ?, ?, ?)", name, "", "GET", "")
        .execute(&mut conn)
        .await
...
async fn get_request(pool: &tauri::State<'_, Pool<Sqlite>>, request_id: i32) -> Result<Request, String> {
...
#[tauri::command]
async fn make_request(pool: tauri::State<'_, Pool<Sqlite>>, request_id: i32) -> Result<String, String> {
    ...
    add_request_history(&pool, RequestHistory {
        id: 0,
        url: url.to_string(),
...
#[tauri::command]
async fn get_request_history(pool: tauri::State<'_, Pool<Sqlite>>, request_id: i32) -> Result<Vec<RequestHistory>, String> {
    let mut conn = pool.acquire().await.map_err(|e| e.to_string())?;
   
    let requests = sqlx::query_as!(RequestHistory, "SELECT * FROM request_history WHERE request_id = ? ORDER BY created_at DESC", request_id)
        .fetch_all(&mut conn)
        .await
        .map_err(|e| e.to_string())?;
...
#[tauri::command]
async fn update_request(pool: tauri::State<'_, Pool<Sqlite>>, request_id: i32, request: Request) -> Result<String, String> {
    let mut conn = pool.acquire().await.map_err(|e| e.to_string())?;
    

    sqlx::query_as!(Request, "UPDATE requests SET url = ?, name = ?, method = ?, headers = ?, body = ? WHERE id = ?", request.url, request.name, request.method, request.headers, request.body, request_id)
        .execute(&mut conn)
        .await
        .map_err(|e| e.to_string())?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    .invoke_handler(tauri::generate_handler![add_request, get_requests, make_request, get_request_history, update_request])
    .manage(pool)
